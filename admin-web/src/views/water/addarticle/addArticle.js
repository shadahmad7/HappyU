...
const addArticle = () => {
  ...
  const onDelete = async (id) => {
    ...
    await fetch('deleteWaterArticle', requestOptions)
  ...
  const submit = async () => {
    ...
    await fetch('addWaterArticle', requestOptions)
      .then((response) => response.json())
      .then((result) => console.log(result))
      .catch((error) => console.log('error', error))
                ...
                <CTableBody>
                  {array.map((item, index) => (
                    <CTableRow v-for="item in tableItems" key={index}>
                      ...
                      <CTableDataCell>
                        <div className="small text-medium-emphasis">{item.water_article_name}</div>
                      </CTableDataCell>
                      <CTableDataCell>
                        <div className="small text-medium-emphasis">
                          <a target="_blank" href={item.water_article_url}>
                            {item.water_article_url}
                          </a>
                        </div>
                      </CTableDataCell>
                      <div className="small text-medium-emphasis">
                          <img src={item.water_article_image} alt="img" width="50" height="50" />
                        </div>
                        <CTableDataCell>
                        <div className="trash" onClick={() => onDelete(item.water_article_id)}>
