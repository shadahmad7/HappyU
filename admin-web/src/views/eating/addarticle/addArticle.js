...
const addArticle = () => {
  ...
  const onDelete = async (id) => {
    ...
    await fetch('deleteEatingArticle', requestOptions)
  ...
  const submit = async () => {
    ...
    await fetch('addEatingArticle', requestOptions)
      .then((response) => response.json())
      .then((result) => console.log(result))
      .catch((error) => console.log('error', error))
                ...
                <CTableBody>
                  {array.map((item, index) => (
                    <CTableRow v-for="item in tableItems" key={index}>
                      ...
                      <CTableDataCell>
                        <div className="small text-medium-emphasis">{item.eating_article_name}</div>
                      </CTableDataCell>
                      <CTableDataCell>
                        <div className="small text-medium-emphasis">
                          <a target="_blank" href={item.eating_article_url}>
                            {item.eating_article_url}
                          </a>
                        </div>
                      </CTableDataCell>
                      <div className="small text-medium-emphasis">
                          <img src={item.eating_article_image} alt="img" width="50" height="50" />
                        </div>
                        <CTableDataCell>
                        <div className="trash" onClick={() => onDelete(item.eating_article_id)}>
