...
const addArticle = () => {
  ...
  const onDelete = async (id) => {
    ...
    await fetch('deleteJournalBook', requestOptions)
  ...
  const submit = async () => {
    ...
    await fetch('addJournalBook', requestOptions)
      .then((response) => response.json())
      .then((result) => console.log(result))
      .catch((error) => console.log('error', error))
                ...
                <CTableBody>
                  {array.map((item, index) => (
                    <CTableRow v-for="item in tableItems" key={index}>
                      ...
                      <CTableDataCell>
                        <div className="small text-medium-emphasis">{item.journal_book_name}</div>
                      </CTableDataCell>
                      <CTableDataCell>
                        <div className="small text-medium-emphasis">
                          <a target="_blank" href={item.journal_book_url}>
                            {item.journal_book_url}
                          </a>
                        </div>
                      </CTableDataCell>
                      <div className="small text-medium-emphasis">
                          <img src={item.journal_book_image} alt="img" width="50" height="50" />
                        </div>
                        <CTableDataCell>
                        <div className="trash"  onClick={() => onDelete(item.journal_book_id)}>
